...
export function AreaForm({
  ...
  const area = api.area.byPublicId.useQuery(
    {
  ...
  const { control, register, reset, handleSubmit, setValue, watch } =
    useForm<AreaFormInput>({
      values: {
        name: isEdit && area.data?.name ? area.data.name : "",
        colour: (isEdit && area.data?.colourCode
          ? colours.find((c) => c.code === area.data?.colourCode)
          : colours[0]) as Colour,
  ...
  const createArea = api.area.create.useMutation({
    onSuccess: (newArea) => {
      ...
      try {
        refetch();
        setModalState("NEW_AREA_CREATED", newArea.publicId);
  ...
  const updateArea = api.area.update.useMutation({
    onSuccess: () => {
      refetch();
      closeModal();
    },
  ...
  const onSubmit = (values: AreaFormInput) => {
    if (!values.colour.code) return;

    if (isEdit) {
      updateArea.mutate({
        areaPublicId: area.data?.publicId ?? "",
        name: values.name,
  ...
  useEffect(() => {
    const nameElement: HTMLElement | null =
      document.querySelector<HTMLElement>("#area-name");
        ...
        <div className="space-x-2">
          {isEdit && (
            <Button
              type="button"
              variant="secondary"
              onClick={() => openModal("DELETE_AREA", entityId)}
            >
              {t`Delete`}
            </Button>
          )}
          <Button
            type="submit"
            isLoading={updateArea.isPending || createArea.isPending}
            disabled={
              !watch("name")
            }
          >
            {isEdit ? t`Update area` : t`Create area`}
          </Button>
        </div>
      </div>
    </form>
